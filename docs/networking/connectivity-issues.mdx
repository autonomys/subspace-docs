...
- <Icon icon={ICONS.CHECK_CIRCLE} /> Peer count > 40 (8–12 peers is sufficient for Chronos phase)
...
# 4. For persistent 0 peers or CGNAT issues (Chronos):
# Replace <BOOTSTRAP_ADDRESS> with the latest addresses from the #announcements channel
./subspace-node run --out-peers 32 --bootstrap-node <BOOTSTRAP_ADDRESS>
```
...
# 2. Increase/Optimize peer connections
# For restrictive NAT/CGNAT, increase outgoing attempts:
./subspace-node run \
  --in-peers 512 \
  --out-peers 32
