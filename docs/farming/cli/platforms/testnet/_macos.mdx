...
<ReleaseContainer>
  ...
  <ReleaseButton to="https://github.com/autonomys/subspace/releases/download/chronos-2026-aug-18/subspace-farmer-macos-aarch64-chronos-2026-aug-18.zip" title="Farmer Executable" subtitle="(Apple CPU)" />
</ReleaseContainer>
...
- `chmod +x subspace-farmer-macos-aarch64-chronos-2026-aug-18`
- `chmod +x subspace-node-macos-aarch64-chronos-2026-aug-18`
...
```bash
./subspace-node-macos-aarch64-chronos-2026-aug-18 \
  run \
  --chain chronos \
  --base-path "<BASE_PATH>" \
  --farmer \
...
```bash
./subspace-farmer-macos-aarch64-chronos-2026-aug-18 farm --reward-address "<REWARD_ADDRESS>" path="<PATH_TO_FARM>,size=<FARM_SIZE>"
```
