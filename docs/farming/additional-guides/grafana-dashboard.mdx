...
- **Block Sync Speed:** Monitor the gap between your local block height and the global network target. A steady increase indicates active catch-up.
  - **Source:** Node (`substrate-node`)
  - **Prometheus Metrics:** `substrate_block_height{type="best"}` (local height) and `substrate_block_height{type="target"}` (network target).
  - **PromQL Gap Example:** `substrate_block_height{type="target"} - substrate_block_height{type="best"}`
