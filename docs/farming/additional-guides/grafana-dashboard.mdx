...
## Monitoring Node Sync & Cache Recovery

When restarting a node or farmer after an extended offline period, the system must rebuild its local cache and sync outstanding data with the network history.

### Key Metrics to Track during Recovery

- **Block Sync Speed:** Monitor the gap between your local block height and the global network target. A steady increase indicates active catch-up.
- **Scrape Target Performance:** Ensure your node (`localhost:9080`) and farmer (`localhost:9081`) endpoints maintain an `UP` state in Prometheus during intense history validation.
- **Telemetry Sync Visibility:** If metrics appear static or do not update instantly, the node is allocating resources to database validation. Allow the cache history to clear the changed data block sequence before restarting the telemetry runtime.
