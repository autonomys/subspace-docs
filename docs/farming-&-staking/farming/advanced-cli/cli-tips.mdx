...
### Speficy exact CPU cores to be used for plotting
This will bypass any custon logic **subspace farmer** might use. 
You can customize the **plotting CPU cores** by `--plotting-cpu-cores`. Cores are comma-sepated, with whitespace separating different thread pools/encoding instances. 
For example, `--plotting-cpu-cores 0,1 2,3` will result in two sectors being encoded at the same time, each with a pair of CPU cores. 


You can also customize the **replotting CPU cores** by `--replotting-cpu-cores` followed by the similar arguements, as in the `--plotting-cpu-cores` example above.

Setting `--plotting-cpu-cores` requires `--replotting-cpu-cores` to be set with the same number of CPU cores groups. If setting `-replotting-cpu-cores` is skipped, the node will use the same thread pool as **plotting**. 
