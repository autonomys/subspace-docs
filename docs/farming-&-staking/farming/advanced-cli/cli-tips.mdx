...
:::note
NUMA support will be beneficial for farmers with a large, powefull CPUs and lots of RAM memory available. Required RAM amount depends on a processor and number of threads, so we can't provide any specific number. 
:::
