...
- **[Polkadot.js Block Explorer](https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Frpc-0.gemini-3h.subspace.network%2Fws#/explorer)**: For users familiar with the Polkadot ecosystem, this explorer offers a seamless experience for exploring the Subspace Network using the Polkadot.js interface.



### Snap Sync

As of the June 11, 2024 version, a new option called Snap Sync is available for fast syncing. It only works for the initial sync, but many people report being fully synced within hours instead of days. The parameter is **--sync snap**. Currently, Snap Sync only works with a farming node, not an operator node.
...
There are cases where you might not want to use Snap Sync. To perform a full sync, use **--sync full**. This is useful if you want to run an RPC node for public use or indexing. In that case, you would need to run an archival node, not just a pruned node. An archival node requires a full sync.
...
:::note
 ...
 The DSN can be a nuanced topic, to better understand our Decentralized Storage Network, please refer to [this guide](https://academy.autonomys.net/subspace-protocol/network-architecture/distributed-storage-network) from our Academy.
