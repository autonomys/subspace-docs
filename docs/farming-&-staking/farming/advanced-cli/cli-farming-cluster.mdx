...
sidebar_position: 2
description: How to setup and configure a Farming Cluster
...
The Cache stores the blockchain's Archive History Size. For optimal performance, the cache must be large enough to hold this data locally. If the data isn't available in the cache, plotting will slow down as it has to be fetched from peers over the DSN. Currently, we recommend a minimum of 200GiB.
...
```mermaid
flowchart
    A[Components] <--> B((NATS Server 1))
    A[Components] <--> C((NATS Server 2))
```
