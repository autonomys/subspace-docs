...
<div className={`${styles.buttons} ${styles.flexContainer}`}>
  <Link
    ...
    to="https://github.com/subspace/pulsar/releases/download/v0.7.4-alpha/pulsar-windows-x86_64-v2-v0.7.4-alpha.exe"
  ...
  <Link
    className="button button--secondary"
    to="https://github.com/subspace/pulsar/releases/download/v0.7.4-alpha/pulsar-windows-x86_64-skylake-v0.7.4-alpha.exe"
    style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '10px' }}>
    <span style={{ fontSize: '20px' }}>Pulsar Executable</span>
...
```powershell
./pulsar-windows-x86_64-skylake-v0.7.4-alpha.exe init
```
...
```powershell
./pulsar-windows-x86_64-skylake-v0.7.4-alpha.exe farm
```
