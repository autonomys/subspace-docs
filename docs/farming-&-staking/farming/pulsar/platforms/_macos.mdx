...
<div className={`${styles.buttons} ${styles.flexContainer}`}>
  <Link
    ...
    to="https://github.com/subspace/pulsar/releases/download/v0.8.0-alpha/pulsar-macos-x86_64-v0.8.0-alpha.zip"
  ...
  <Link
    className="button button--secondary"
    to="https://github.com/subspace/pulsar/releases/download/v0.8.0-alpha/pulsar-macos-aarch64-v0.8.0-alpha.zip"
    style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '10px' }}>
    <span style={{ fontSize: '20px' }}>Pulsar Binary</span>
...
```shell-session
./pulsar-macos-x86_64-v0.8.0-alpha  init
```
...
```shell-session
./pulsar-macos-x86_64-v0.8.0-alpha farm
```
