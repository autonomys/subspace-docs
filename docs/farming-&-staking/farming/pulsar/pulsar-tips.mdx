...
There are some cases where version updates will cause issue with **Pulsar** and you may have to use the `./pulsar_release wipe` command, typically when errors occur. 
...
To wipe the node and plots for the Pulsar, run the following command:
