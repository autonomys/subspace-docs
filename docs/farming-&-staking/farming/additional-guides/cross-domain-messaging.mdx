...
Currently there are no additional fees for using XDM beyond the standard transaction fees on each chain. However, there is a small execution fee charged in tAI3 for processing the XDM transaction on the destination chain.

## Executing XDM Transfers

### Consensus to Auto EVM domain with PolkadotJS

You will need to navigate to the Auto EVM domain in PolkadotJS by either selecting it in the network switcher or following this link: https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Frpc.mainnet.autonomys.xyz%2Fws#/extrinsics

PolkadotJS must be linked to a wallet that has some tAI3 tokens on the consensus chain. You can use either [SubWallet](/wallets/subwallet) or [Talisman Wallet](/wallets/talisman) for this.
...
- Platform: Ethereum
- RPC Url: https://rpc.mainnet.autonomys.xyz
- Network Name: Auto EVM Mainnet (Or any name you prefer)
- Native Token Symbol: AI3 (Or any name you prefer)
...
1. Select the account you want to consensus tokens from.
2. Select the `transporter.transfer()` extrinsic.
3. Select `Domain` in the `dstLocation` `chainId`.
...
6. Enter the `0x` account address you want to send the tokens to on the Auto EVM domain.
...
![Convert Public Key](/img/doc-imgs/additional-guides/XDM-2.png)

Next, you will need to navigate to the Auto EVM domain in PolkadotJS by either selecting it in the network switcher or following this link: https://polkadot.js.org/apps/?rpc=wss%3A%2F%2Frpc.mainnet.autonomys.xyz%2Fws#/extrinsics
