...
Currently, there are no additional fees for using XDM beyond the standard transaction fees on each chain. However, there is a small execution fee charged for processing the XDM transaction on the destination chain.
