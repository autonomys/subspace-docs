...
For security reasons, RPC ports 9944 and 9945 should never be exposed to the internet via port forwarding unless you're operating a public RPC server. However, if the node and farmer are on different computers, these ports should be allowed through the firewall on the node.
