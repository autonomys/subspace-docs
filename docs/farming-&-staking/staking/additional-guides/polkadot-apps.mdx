...
5. Set the nomination `amount` in **Shannons**. For reference, 100 AI3/tAI3 equals 100 followed by 18 zeros.  
...
5. Set the initial operator stake `amount` in **Shannons**. For reference, 100 AI3/tAI3 is represented as 100 followed by 18 zeros. The value must be an integer equal to or greater than 100 tokens.
6. Set the minimum nominator stake required to participate under this operator in **Shannons**. For reference, 10 AI3/tAI3 is represented as 10 followed by 18 zeros. The value must be an integer equal to or greater than 1 token.
...
4. Enter the `OperatorId` you wish to nominate.  
5. Select `All` to withdraw the full amount, or choose `Stake` to withdraw only a portion. If you choose to withdraw a partial stake, enter 80% of the amount you wish to withdraw. The remaining 20% is held in a storage fund and will be refunded automatically. For example, to withdraw 100 AI3/tAI3, you would enter (100 * 0.80), which equals 80 followed by 18 zeros as the amount is in **Shannons**.
6. Click `Submit Transaction` and sign the extrinsic with your supported [wallet](/wallets).
