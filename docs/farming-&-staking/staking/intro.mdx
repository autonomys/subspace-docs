...
Running an Operator is not currently available to the community. At this time, only Subspace Labs and the Subspace Foundation are running Operators. Community access to Operators will be enabled on Chronos in the near future, with mainnet access to follow.
