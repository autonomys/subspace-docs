...
:::info
It can take up to 10 minutes for the operator to be registered and appear on the page.  It may take a 1-2 days (depending on block production speed) before your Stake amount is displayed under Total Stake in the table.  Once you have your Operator Id you can restart your operator node and include the `--operator-id **OPERATOR_ID**` as a parameter. 
:::
...
3. After approving the transaction, you will then see a pending transaction under your operator that should be completed in about 10 minutes.
