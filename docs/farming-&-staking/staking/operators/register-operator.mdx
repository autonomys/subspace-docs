...
Currently, only running and registering an operator on the Taurus network is supported.
:::
...
7. Select your Domain (It will default to the Taurus EVM), Signing key, Amount to Stake, Nominator Tax, a Minimum Nominator Stake, and then click on the `Register Operator` button at the bottom of the page.
