...
1. Start a node on a developer chain by running `./target/release/subspace-node -- --domain-id 0 --keystore-path tmp/keystore --rpc-cors all`
...
`./target/release/subspace-node --dev -- --domain-id 0 --keystore-path tmp/keystore --rpc-cors all`
