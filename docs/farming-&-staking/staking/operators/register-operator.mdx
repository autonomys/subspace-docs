...
It is possible to register an operator for each domain, but each operator will require running it's own node. Currently operators can only be registered on the Taurus network.
...
5. Proceed to the `Staking` tab and `Register Operator`.
...
7. Congratulations, you're now registered as an **operator**!  You can click on the Manage My Operator tab to see your operator and get the Operator Id.
