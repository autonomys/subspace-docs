...
<ContentGroup title="Farming" icon={<Icon icon={ICONS.FARMER} />} id="farming">
  ...
  <ContentBlock 
    ...
    description="If you need to change the default ports for the farmer and node, whether to run multiple instances on the same local network or to operate both the mainnet and the Taurus testnet at the same time, you must configure them to use unique ports."
  >
    <Tabs>
