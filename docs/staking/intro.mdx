...
Running an operator is not currently available to the community. At this time, only Autonomys Labs and the Subspace Foundation are running operators. Community access to operators will be enabled on Chronos in the near future, with mainnet access to follow.

In the meantime, staking to an operator is already available on mainnet through the [Staking Portal](https://staking.autonomys.xyz/) - see the [Stake with the Staking Portal](/staking/staking-portal) <Badge variant="recommended" text="Recommended" /> guide.
:::
...
:::tip Ready to stake?
- [Stake with the Staking Portal](/staking/staking-portal) <Badge variant="recommended" text="Recommended" /> - the friendly interface at [staking.autonomys.xyz](https://staking.autonomys.xyz) that walks you through staking and unstaking.
- [Stake with the Substrate Portal](/staking/substrate-portal) - submit the staking extrinsics manually via Polkadot.js Apps.
:::
