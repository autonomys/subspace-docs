...
<TabItem value="docker" label="🐋 Docker" default>
  ...
  2. Create the `subspace` directory and the `docker-compose.yaml` file in it. Paste the output from the generator into the file.
  
  3. Now go to directory with `docker-compose.yaml` and type `docker-compose up -d` to start everything
