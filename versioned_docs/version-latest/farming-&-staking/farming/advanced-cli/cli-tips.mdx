...
At the moment, Subspace Node supports two implementation mechanisms:

- `single`, that opens the file once and uses that for reading from multiple threads.
- `rayon`, that opens files as many times as there are farming threads and, for each thread, uses a dedicated file handle. **Rayon** is the library we use for multi-threading stuff.

To interpret the results: typically, you assess throughput to determine the required size of the SSD for efficient auditing. Both CPU and disk performance influence this metric, so adding a second disk of the same size may not necessarily double the time; its impact is likely to be less significant. Nonetheless, this approach provides you with a general understanding of performance.
