...
<div className={`${styles.buttons} ${styles.flexContainer}`}>
  <Link
    ...
    to="https://github.com/subspace/pulsar/releases/download/v0.7.1-alpha/pulsar-macos-x86_64-v0.7.1-alpha.zip"
  ...
  <Link
    className="button button--secondary"
    to="https://github.com/subspace/pulsar/releases/download/v0.7.1-alpha/pulsar-macos-aarch64-v0.7.1-alpha.zip"
    style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '10px' }}>
    <span style={{ fontSize: '20px' }}>Pulsar Binary</span>
...
```shell-session
./pulsar-macos-x86_64-v0.7.1-alpha  init
```
...
```
5. Once complete, the settings will be written to the `settings.toml`. You can find Your `settings.toml` in `$HOME/Library/Application Support/pulsar/settings.toml`
...
```shell-session
./pulsar-macos-x86_64-v0.7.1-alpha farm
```
