...
The example is not Stake Wars specific, the operator is responsible for finding out the correct domain ID they want to operate on. **Stake Wars are using the domains with ID's 0 (Nova) and 1 (Auto-ID)**.
:::
...
Once you click the Generate proof button it will automatically populate the `Signing key` and `Prof of signing key ownership signature` fields.  Enter the `Minimum Operator Stake`, `Amount to Stake`, `Nomination Tax` and then click `Next`.

![NStaking-8](/img/doc-imgs/operators-staking/NStaking-8.png)

:::info
Make sure to either locate the signing key file generated on the previous **[Create operator key](#create-operator-key)** step or get use the key phrase that was displayed when the key was created and is also in that key file.
:::
