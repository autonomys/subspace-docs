...
Make sure to use the base **--base-path** for the key generation, and running an operator node and make sure to replace **--domain-id value** with the domainID you would like to be an operator on.
