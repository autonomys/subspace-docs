...
If you prefer to build from the source rather using existing builds, the domain operator node is embedded within the `subspace-node` binary, please refer to [Subspace node](https://github.com/autonomys/subspace/blob/main/crates/subspace-node/README.md) for how to build from source.
