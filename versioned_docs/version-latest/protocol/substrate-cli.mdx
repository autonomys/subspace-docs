...
:::note Feedback!
Your opinions on our releases contribute to our progress. Fill out this [form](https://c6gxctjlh68.typeform.com/to/Ks4Y16Fo) and your voice will be valued and counted.
:::
...
:::tip [Releases](https://github.com/subspace/subspace/releases) section on our GitHub
the best place to get fresh releases of binaries and source code
:::
...
<TabItem value="linux" label="🐧Ubuntu (Home PC)">
  <details>
    <summary>
...
<TabItem value="systemd_service" label="⚙ Linux Server">



Adhering to standard Linux conventions, we place executable files in `/usr/local/bin`, create dedicated directories for their data, and run services from a special user.

All further actions are performed under a regular user.

### Creating a user with restricted rights to run the service
```console
sudo useradd --no-create-home --shell /bin/false subspace
```
...
Commands below will download the binaries to your current folder, renaming them to more convenient manner:
...
Copy them to `/usr/local/bin` and delete originals:  
```console
sudo cp subspace-node subspace-farmer /usr/local/bin
```
Make them executable:
```console
sudo chmod +x /usr/local/bin/subspace-node /usr/local/bin/subspace-farmer
```
*However, you can download binaries directly to the right place if you run these commands from `/usr/local/bin` directory with root privilegies.*

Set ownership to user 'subspace':
```console
sudo chown subspace:subspace /usr/local/bin/subspace-node
sudo chown subspace:subspace /usr/local/bin/subspace-farmer
```

:::tip You can use the same commands for updating the node and the farmer
:::


### Creating dedicated directories
This command creates `subspace-node` and `subspace-farmer` directories for node's data and farmer's data respectively with owning by user 'subspace':
```console
sudo -u subspace mkdir /var/lib/subspace-node /var/lib/subspace-farmer
```

If your plots are intended to reside on external disks, also change the owner of the corresponding directories to user subspace, for example:
```console
...
---------------------------
---------------------------
## TEMPLATE VERSION (just to test how it looks)

#### subspace-node.service  
Copy this to your /etc/systemd/system/subspace-node file:
```console
sudo vi /etc/systemd/system/subspace-node
```
...
Copy this to your /etc/systemd/system/subspace-farmer file:  
```console
sudo vi /etc/systemd/system/subspace-farmer
```
...
ExecStart=/usr/local/bin/subspace-farmer  \  
    farm \  
    --reward-address INSERT_HERE_YOUR_WALLET_ADDRESS \  
    --listen-on /ip4/0.0.0.0/udp/30533/quic-v1 \  
    --listen-on /ip4/0.0.0.0/tcp/30533 \  
    path=/var/lib/subspace-farmer,size=600G  
KillSignal=SIGINT  
Restart=always  
RestartSec=10  
Nice=-5  
...
:::note
In to this template insert your wallet address; paths to all you plots with their sizes, if you have multi-disk setup; and play with some recommended [parameters](https://docs.subspace.network/docs/protocol/substrate-cli/#optimizing-dsn-syncing)
:::

#### Service MUST be granted execute permission!
```console
sudo chmod +x /etc/systemd/system/subspace*service
```
...
:::note NOTICE THE "SANDBOX" BUTTON in the form below.
It is active when the checkbox color differs from the background color.
:::  


<AdvancedSystemdServiceFileGenerator />

#### Creating the Node Service File:
```shell-session
sudo vi /etc/systemd/system/subspace-node.service
```  
 And paste the corresponding generated content to it.


#### Creating the Farmer Service File:
```shell-session
sudo vi /etc/systemd/system/subspace-farmer.service
```  
 Add your paths for multi-disk setup, and paste the corresponding generated content to it.
...
### Service Management Commands (replace ꜱᴇʀᴠɪᴄᴇ with subspace-node or subspace-farmer):

Enable and immediately start node and farmer:
```shell-session
sudo systemctl enable --now subspace-{node,farmer}
```
Start ꜱᴇʀᴠɪᴄᴇ: 
```shell-session
sudo systemctl start ꜱᴇʀᴠɪᴄᴇ
```
Stop ꜱᴇʀᴠɪᴄᴇ:
```shell-session
sudo systemctl stop ꜱᴇʀᴠɪᴄᴇ
```
Enable 𝕤𝕖𝕣𝕧𝕚𝕔𝕖 (for automatic startup on system boot):
```shell-session
sudo systemctl enable 𝕤𝕖𝕣𝕧𝕚𝕔𝕖
```
Disable 𝕤𝕖𝕣𝕧𝕚𝕔𝕖 (to prevent automatic startup on system boot):
```shell-session
sudo systemctl disable 𝕤𝕖𝕣𝕧𝕚𝕔𝕖
```
Check 𝕤𝕖𝕣𝕧𝕚𝕔𝕖 Status:
```shell-session
sudo systemctl status 𝕤𝕖𝕣𝕧𝕚𝕔𝕖
```
View Node Logs:
```shell-session
