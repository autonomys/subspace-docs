...
:::
  ...
  :::note Farming while Plotting changes
  :::
   ```bash
    # Replace `PATH_TO_FARM` with location where you want you store plot files
    # Replace `WALLET_ADDRESS` below with your account address from Polkadot.js wallet
...
Gemini-3g Introduces our Proof-of-Time, a new optional role that has been added to the node. The Timekeepers help run PoT on the network to ensure the security of the network. Timekeeping requires a high-performance core cpu but can be ran by any node runner. You can enable time keeping with the following commands. 
