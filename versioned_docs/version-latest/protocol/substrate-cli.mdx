...
:::note Feedback!
Your opinions contribute to our progress. Fill out this [form](https://c6gxctjlh68.typeform.com/to/Ks4Y16Fo) and your voice will be valued and counted.
:::
...
:::note Farming can be Network Intensive.

During the plotting phase, Farmers may operate with a high network load.  
This can affect network utilization, kindly check if you have hard data limit set.
:::
...
It is recommended not to use Subspace Farmer and Node with CoW (Copy on Writes) enabled. That could be in ReFS, BTRFS, ZFS, bcachefs, HAMMER and some other file systems.

In case of using BTRFS, the directory/whole file system must be CoW disabled with the following command prior to starting Subspace, because it only applies to newly created files.
...
:::note Redundancy
Using redundancy (RAID1/5/6, RAIDZ and others) on a filesystem with plot makes no sense, as Subspace farm can make better performance decisions by seeing how many physical disks you have and parallelizing things accordingly. Farmer also uses checksums to verify the integrity of the plot and in most cases is able to repair damaged blocks if necessary. See the `scrub` command below.
:::
...
Before running anything you need to have a wallet where you'll receive testnet coins. If you do not already have a wallet, please use the instructions in [this section](https://docs.subspace.network/docs/category/wallets).
Additional info may be found in our forum post [How to setup Subwallet & a Polkadot.js Wallet](https://forum.subspace.network/t/subspace-wallet/61)
...
:::tip 
[Releases](https://github.com/subspace/subspace/releases) section on our GitHub is the best place to get fresh releases of binaries and source code
:::
...
For Docker nothing needs to be downloaded, just configure `docker-compose.yaml`
...
:::caution
  ...
  The limit can generally be found on the path /proc/sys/fs/file-max. It is recommended to change the limit to 100k if it is not already in place. Here is a [tutorial.](https://www.tutorialspoint.com/increase-number-of-maximum-open-files-in-linux)
...
### Running Node and Farmer in Tmux/Screen Session

Create a new `tmux` session using a socket file named `farming`:
```shell-session
tmux -S farming
```  
Once the Tmux session is created, you can go to executable directory and run the farming process.
```shell-session
./subspace-cli farm
```
Attach session:
...
Example with Screen. This will create a new Screen session:
```shell-session
screen -S farming
./subspace-cli farm
```
...
<TabItem value="docker" label="🐋 Docker" default>
  ...
  1. Enter your data in the form provided and click *Generate*.  
  Please use your reward address instead of the default one!
	 
  <DockerFileGenerator />
  
  2. Copy the generator's output into your `docker-compose.yaml`.

  3. Use `docker compose up -d` in the directory with your `docker-compose.yaml` to start everything!
...
<TabItem value="service" label="Service(Linux)">



### Creating a Subspace Dedicated User
...
:::tip 
The [Releases](https://github.com/subspace/subspace/releases) section on our GitHub is the best place to find the latest releases of binaries and source code.
:::

The commands below download binaries to `/home/subspace/.local/bin`, assigning them more convenient names.
Please use the appropriate commands. You can change the directory by modifying the respective string.
...
<details>
   <summary>
    ...
    <details>
		...
		wget -O ~/.local/bin/subspace-node https://github.com/subspace/subspace/releases/download/gemini-3g-2023-nov-21/subspace-node-ubuntu-x86_64-v2-gemini-3g-2023-nov-21
		</CodeBlock>
		Farmer:
		<CodeBlock language="shell-session">
		wget -O ~/.local/bin/subspace-farmer https://github.com/subspace/subspace/releases/download/gemini-3g-2023-nov-21/subspace-farmer-ubuntu-x86_64-v2-gemini-3g-2023-nov-21
		</CodeBlock>
    </details>
    <details>
        <summary>
		...
		Node:
		<CodeBlock language="shell-session">
		wget -O ~/.local/bin/subspace-node https://github.com/subspace/subspace/releases/download/gemini-3g-2023-nov-21/subspace-node-ubuntu-x86_64-skylake-gemini-3g-2023-nov-21
		</CodeBlock>
		Farmer:
		<CodeBlock language="shell-session">
		wget -O ~/.local/bin/subspace-farmer https://github.com/subspace/subspace/releases/download/gemini-3g-2023-nov-21/subspace-farmer-ubuntu-x86_64-skylake-gemini-3g-2023-nov-21
		</CodeBlock>
    </details>
</details>
...
<details>
    ...
    Aarch64/64-bit Raspberry Pi Executables
		...
		Node:
		<CodeBlock language="shell-session">
		wget -O ~/.local/bin/subspace-node https://github.com/subspace/subspace/releases/download/gemini-3g-2023-nov-21/subspace-node-ubuntu-aarch64-gemini-3g-2023-nov-21
		</CodeBlock>
		Farmer:
		<CodeBlock language="shell-session">
		wget -O ~/.local/bin/subspace-farmer https://github.com/subspace/subspace/releases/download/gemini-3g-2023-nov-21/subspace-farmer-ubuntu-aarch64-gemini-3g-2023-nov-21
		</CodeBlock>
</details>

:::tip
You can use the same commands for updating the node and the farmer
:::
...
### Directories for Node and Farmer Data

```shell-session
mkdir -p ~/.local/share
mkdir -p ~/.local/share/subspace-node
mkdir -p ~/.local/share/subspace-farmer
```
...
exit
```
If your plots are intended to reside on external disks, make sure that the user Subspace has the permission to write to these directories, for example:
```shell-session
sudo chown -R subspace:subspace /mnt/farmer-1
sudo chown -R subspace:subspace /mnt/farmer-2
sudo chown -R subspace:subspace /mnt/farmer-3
```

:::tip
If you have an external journaling partition, you do not need to assign a user 'subspace' ownership to it. It is managed by the file system driver and does not require direct user ownership or interaction.
:::
...
Service File Generator below generates contents for the service files.
:::note 
Please replace the relevant fields: Node Name, Reward Address, Plot Size, paths to binaries and data (remember to add paths for multi-disk setup), System User.   
Notice the "Sandbox". It is active by default. When the sandbox is active - the generated files will contain noticeably more lines.
:::  


<AdvancedSystemdServiceFileGenerator />

Open a file for editing (Node) and copy the generated content into it:
```shell-session
EDITOR=nano sudo -e /etc/systemd/system/subspace-node.service
```  
 Open a file for editing (Farmer) and copy the generated content into it:
```shell-session
EDITOR=nano sudo -e /etc/systemd/system/subspace-farmer.service
```

Whenever the service files are edited, reload the daemon:
```shell-session
...
Enable and immediately start Node and Farmer services:
```shell-session
sudo systemctl enable --now subspace-{node,farmer}
```
Start a service: 
```shell-session
sudo systemctl start 'service-file-name'
```
Stop a service:
```shell-session
sudo systemctl stop 'service-file-name'
```
Enable a service (for automatic startup on system boot):
```shell-session
sudo systemctl enable 'service-file-name'
```
Disable a service (to prevent automatic startup on system boot):
```shell-session
sudo systemctl disable 'service-file-name'
```
Check status:
```shell-session
...
```
and proceed from the beginning of this manual.
...
In case of Docker setup run `docker compose down -v` (and manually delete custom directories for volumes, if you have specified them):

##### Some commands that may help to clean it proparly:

``sudo du -aBm / 2>/dev/null | sort -nr | head -n 10`` # 10 biggest files in System  
``docker compose down -v`` # to wipe plots  
``docker stop $(docker ps -aq)`` # stop all containers  
``docker rm $(docker ps -aq)`` #delete all containers;  `-q` flag tells Docker to only display the numeric IDs (IDs)  
``docker rmi -f $(docker images -a -q)`` # delete all images  
``docker system prune -a --volumes`` # another way to delete all images  
``docker image prune -a`` # del all unused images  
``docker system prune``  # Reclaims disk space by removing containers, networks, volumes, and images that are not being used by any running containers.  
``docker info	`` # check...  

##### To try something more ambitious, you can run an Ubuntu container with:  
``docker run -it ubuntu bash``			# login to Docker mini-system...  
``docker exec -it cd8e438ba00a <whatever_you_want_to_run>`` for ex. to get shell: ``docker exec -it cd8e438ba00a /bin/bash``  
...
:::
#### Recommended Parameters to "Play With":
...
It is highly recommended to consult [Official LLVM guide](https://apt.llvm.org/#llvmsh)

Checking your versions:
```shell-session
clang -v
```
(by default you most likely have low clang version which is subject to upgrade. You have to uninstall older versions of clang tools manually).
...
If you are having issues with running the node or the farmer for the subspace network, feel free to join our Discord or even better you can take a look at our Forums and review and existing questions or post a new one if needed!
